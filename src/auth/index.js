...
export const useAuth0 = ({
    ...
    instance = new Vue({
        ...
        methods: {
            ...
            async createCompanyUser(user) {
                var roles = user[this.roleKey] || [];
                if (!roles.includes("Company")) {
                    roles.push("Company");
                }
                try {
                    let response = await this.$axios.post(this.$apiBase + '/v1/users', {
                        auth_id: user.sub,
                        name: user.name,
                        email: user.email,
        ...
        async created() {
            ...
            try {
                ...
                if (this.user) {
                    ...
                    this.companyId = this.user[this.companyIdKey];
                    // get access token
